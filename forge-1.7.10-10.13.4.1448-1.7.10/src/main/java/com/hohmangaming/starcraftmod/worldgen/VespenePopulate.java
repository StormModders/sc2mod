...
//		BuildCraftCore.debugMode = true;
	...
	public void populate(PopulateChunkEvent.Pre event) {
		...
		generateVespian(event.world, event.rand, event.chunkX, event.chunkZ);
	}

	public void generateVespian(World world, Random rand, int chunkX, int chunkZ) {
				...
				for (int poolY = -radius; poolY <= radius; poolY++) {
					for (int poolZ = -radius; poolZ <= radius; poolZ++) {
						...
						if (distance <= radiusSq) {
							world.setBlock(poolX + wellX, poolY + wellY, poolZ + wellZ, StarcraftMod.blockVespene.blockID, 0, distance == radiusSq ? 3 : 2);
						}
					}
				}
			}
...
//			}
			...
			if (type == GenType.LARGE) {
				for (int y = wellY; y <= maxHeight - wellHeight / 2; ++y) {
					world.setBlock(wellX + 1, y, wellZ, StarcraftMod.blockVespene.blockID);
					world.setBlock(wellX - 1, y, wellZ, StarcraftMod.blockVespene.blockID);
					world.setBlock(wellX, y, wellZ + 1, StarcraftMod.blockVespene.blockID);
					world.setBlock(wellX, y, wellZ - 1, StarcraftMod.blockVespene.blockID);
				}
			}
	...
	private boolean isVespene(World world, int x, int y, int z) {
		int blockId = world.getBlockId(x, y, z);
		return (blockId == StarcraftMod.blockVespene.blockID);
	}
	...
	private void setVespeneColumnForLake(World world, BiomeGenBase biome, int x, int y, int z, int depth, int update) {
		if (isReplaceableForLake(world, biome, x, y + 1, z)) {
			...
			if (isReplaceableFluid(world, x, y, z) || world.isBlockSolidOnSide(x, y - 1, z, ForgeDirection.UP)) {
				world.setBlock(x, y, z, StarcraftMod.blockVespene.blockID, 0, update);
			} else {
				return;
			}
			...
			for (int d = 1; d <= depth - 1; d++) {
				...
				world.setBlock(x, y - d, z, StarcraftMod.blockVespene.blockID, 0, 2);
			}
		}
	}
